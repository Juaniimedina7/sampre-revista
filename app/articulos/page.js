...
import { Search, X, Filter, FileText, ChevronRight, BookOpen } from 'lucide-react'
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex-1 min-w-0">
              ...
              {/* Estado vacío — sin artículos aún */}
              {articles.length === 0 && (
                <div className="text-center py-20">
                  ...
                  <h2
                    className="text-xl font-bold text-journal-navy mb-2"
                    style={{ fontFamily: 'var(--font-display)' }}
                  >
                        ...
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex flex-wrap items-center gap-2 mb-2">
                              ...
                              {article.pages && (
                                <span className="text-xs text-gray-400">Págs. {article.pages}</span>
                              )}
                            </div>

                            {/* Título */}
                            <h3
                              className="font-bold text-gray-900 group-hover:text-primary-700 transition-colors mb-1 leading-snug"
                              style={{ fontFamily: 'var(--font-display)' }}
                            >
                              {article.title}
                            </h3>
                            {article.titleEn && (
                              <p className="text-sm text-gray-400 italic mb-2">{article.titleEn}</p>
                            )}
                            ...
                            {article.keywords?.length > 0 && (
                              <div className="flex flex-wrap gap-1.5">
                                {article.keywords.map((kw) => (
                                  <span key={kw} className="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded">
                                    {kw}
                                  </span>
                                ))}
                              </div>
                          ...
                          <ChevronRight className="w-5 h-5 text-gray-300 group-hover:text-primary-600 flex-shrink-0 mt-1 transition-colors" />
                        </div>
                      </Link>
                    )
                  })}
